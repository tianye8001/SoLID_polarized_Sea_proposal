...
void analysis_3He_d2n_11GeV_LA(string input_filename){
gROOT->Reset();
gStyle->SetPalette(1);
gStyle->SetOptStat(0);
gStyle->SetOptFit(0);
...
TFile *file1;
TH2F *hf,*hl,*hall;
file1 = new TFile("/work/halla/solid/tianye/solid_svn/study/acceptance/result_SIDIS_He3/201701/acceptance_solid_SIDIS_He3_electron_201701_1e7_output_final.root");
//file1 = new TFile("/work/halla/solid/tianye/solid_svn/study/acceptance/result_SIDIS_He3/201402/acceptance_solid_CLEO_SIDIS_3he_negative_output.root");
  ...
  hall = (TH2F*)file1->Get("acceptance");*/
const int m=12;
...
//}
  ...
  Q2_Xjb_rate_hist = new TH2F("Q2_Xjb_rate_hist","Q2_Xjb_rate_hist",220,0,11,200,0,1);
  Q2_rate_hist = new TH1F("Q2_rate_hist","Q2_rate_hist",9,1,10);
...
//         if(dxs_gen>10 && Q2_gen>1){
          //cout<<"event="<<i<<"rate="<<rate<<"dxs_gen="<<dxs_gen<<endl; 
//         }

       bin=hl->FindBin(theta_gen,p_gen);
       hl->GetBinXYZ(bin,binx,biny,binz);
      if(pid_gen==11 && Q2_gen>1 && p_gen>3.0){
     ...
     // double genvar[4] = {Q2_gen,x,theta_gen,phi_value}; 
      ...
      double weight = rate_gen*time; 
      Q2_Xjb_theta_mom_gen_hist->Fill(genvar,weight);
      if(accep_e_2>0){
      //if(accep_e_1>0){
        double weight_EC=rate_gen*time*accep_e_2;
      //cout<<"event="<<i<<"acceptance="<<accep_e_2<<"rate_gen="<<weight_EC<<"p_gen="<<p_gen<<"theta_gen="<<theta_gen<<endl; 
        ...
        Q2_Xjb_rate_hist->Fill(Q2_gen,x,weight_EC);
        Q2_rate_hist->Fill(Q2_gen,weight_EC);
      }
  }
 }
file->Close();

TFile *outputfile_final=new TFile("analysis_11GeV_LA_threshold3GeV_noWcut_Longitudinal.root", "recreate");
outputfile_final->Append(Q2_Xjb_theta_mom_gen_hist);
outputfile_final->Append(Q2_Xjb_dxs_hist);
outputfile_final->Append(Q2_Xjb_rate_hist);
outputfile_final->Append(Q2_rate_hist);
